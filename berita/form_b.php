...
{    
    ...
    $getId=mysqli_fetch_row(mysqli_query($con,"select max(ID_News) from news"));
    
    
    
    mysqli_query($con,"insert into news values('','".date('Y-m-d')."','".($getId[0]+1).$ext."','$Title','$Content')");
    
    
      echo "
    <script>
    location.assign('indexberita.php?page=berita&ps=true1');
    </script>
    ";
}
elseif(isset($_POST['update']))
{
        ...
        mysqli_query($con,"update news set Title='$Title',Content='$Content' where id='".$_GET['id']."'");
    
    echo "
    <script>
    location.assign('indexberita.php?page=berita&ps=true2');
    </script>
    ";
}
...
if(isset($_GET['id']))
$data=mysqli_fetch_row(mysqli_query($con,"select * from news where ID_News='".$_GET['id']."'"));
