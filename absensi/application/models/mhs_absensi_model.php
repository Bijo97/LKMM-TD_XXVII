...
class mhs_absensi_Model  extends CI_Model  {
	...
	{
		$query=$this->db->query("select count(*) as a from mahasiswa where nrp={$nrp}");
		return $query->row();
	}
	function count_masuk($nrp,$id)
	{
		$query=$this->db->query("select count(*) as a from mhs_absen where nrp=$nrp and id_keg={$id}");
		return $query->row();
	}
	...
	function insert($nrp,$id,$sekret,$hadir){
		$data = array(
		   'nrp'=>$nrp,
		   'id_keg'=>$id,
		   'nrp_sekret'=>$sekret,
	...
	function insert_new($nrp,$id,$sekret,$hadir){
		$data = array(
		   'nrp'=>$nrp,
		   'id_keg'=>$id,
		   'nrp_sekret'=>$sekret,
